...
highlightSelectedContact(selected, selectedMail);
adjustDisplay(display);
display.innerHTML = renderInfoTemplate(name, email, color, phone);
adjustScrollAndArrow();
}

/**
 * this fucntion is used to highlight the selected contact
 * @param {div} selected the display area
 * @param {div} selectedMail the selected contact
 */
function highlightSelectedContact(selected, selectedMail) {
...
/**
 * this function is used to animate the rendering of the selected
 * @param {div} display the display area 
 */
function adjustDisplay(display) {
  const winWidth = window.innerWidth;
  if (winWidth > 1040) {
      display.style.display = "none";
      display.style.alignItems = 'flex-end';
      setTimeout(() => {
          display.style.display = "flex";}, 200);
      setTimeout(() => {
          display.style.alignItems = 'center';}, 250);
      setTimeout(() => {
          display.style.alignItems = 'flex-start';}, 300);
  } else {
...
/**
 * this function is used to adjust the back arrow
 */
function adjustScrollAndArrow() {
  const winWidth = window.innerWidth;
